...
fl_ruta_file = Path("C:/System_VideoClub/DirectoresCine.txt")
...
# director de peliculas del sistema.
def registrarDatos():
    print("Ingrese los datos del nuevo director: ")
    for clave in Dcc_DirectorSystem:
        if format(clave) == "Id_Director":
            print(f"{clave}:___")
            valor = ""
        ...
        Dcc_DirectorSystem[clave] = valor

    while True:
        Lv_Guardar = str(input("Desea guardar los datos del director (S/N): ")).upper()
        if Lv_Guardar == "S":
            print("Procesando...")
            # Proceso de almacenamiento
            # Recuperamos la secuencia de los registros guardados
            Ln_SenccReg = util.Prc_ContrSecuencia("DirectorCine")
            # Salvo el registro dentro del file del archivo
            with open(fl_ruta_file, "a") as archivoUser:
                Linea = "Director" + str(Ln_SenccReg) + ":"
                for clave, valor in Dcc_DirectorSystem.items():
                    if format(clave) == "Id_Director":
            ...
            Lv_Guardar = util.Prc_UpdateSecuencia("DirectorCine", Ln_SenccReg)
            if Lv_Guardar == "SI":
                print("La secuencia de los registros de director fue actualizada")
            else:
